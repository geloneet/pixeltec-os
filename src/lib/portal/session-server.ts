...
export async function createPortalSession(clientId: string, slug: string): Promise<void> {
  const now = Math.floor(Date.now() / 1000);
  const payload: PortalSessionPayload = { clientId, slug, iat: now, exp: now + TTL_SECONDS };
...
export async function readPortalSession(): Promise<PortalSessionPayload | null> {
  ...
  try {
    const payload: PortalSessionPayload = JSON.parse(
...
/** Sliding expiration: re-emits the cookie on every valid hit, renewing the TTL. */
export async function refreshPortalSession(): Promise<PortalSessionPayload | null> {
  const session = await readPortalSession();
  if (!session) return null;
  await createPortalSession(session.clientId, session.slug);
  return session;
}
