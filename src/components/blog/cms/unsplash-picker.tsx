...
/**
 * Buscador de portadas Unsplash para el editor de blog (WO-2026-00198).
 * Adaptado del picker legacy del blog-admin retirado (commit `b212c9f`, el
 * canal de egress `unsplash` y `searchCoverImages` nunca se borraron: solo la
 * UI del blog-admin viejo se fue con WO-2026-00132). Backend sin cambios.
 */
 ...
 *  canal de egress; sus errores se muestran tal cual dentro del dialog (p. ej.
 *  «agrega UNSPLASH_ACCESS_KEY y EGRESS_UNSPLASH_MODE=live» sin configurar). */
export function UnsplashPicker({ onSelect, triggerLabel = "Buscar en Unsplash" }: UnsplashPickerProps) {
  ...
  const canLoadMore = photos.length > 0 && page < MAX_PAGE && lastBatchCount === PAGE_SIZE;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="border-border bg-background text-foreground sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Buscar portada en Unsplash</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Elige una foto para usarla como imagen de portada de la entrada.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex gap-2">
          <Input
            ...
            autoFocus
          />
          <Button type="button" onClick={() => runSearch(1, false)} disabled={isSearching || query.trim().length < 2}>
        ...
        <div className="max-h-[55vh] overflow-y-auto">
          {isSearching && photos.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-3">
              {photos.map((photo) => (
                <button key={photo.id} type="button" onClick={() => handleSelect(photo)} className="group text-left">
                  <div className="relative aspect-video w-full overflow-hidden rounded-lg border border-border transition group-hover:border-cyan-500/50 group-hover:opacity-90">
                    <Image
                      src={photo.thumbUrl}
                  ...
                  <p className="mt-1 truncate text-xs text-muted-foreground">Foto: {photo.authorName}</p>
                </button>
              ))}
            </div>
          ) : hasSearched && !error ? (
            <p className="py-12 text-center text-sm text-muted-foreground">Sin resultados para «{lastQuery}». Prueba otra búsqueda.</p>
          ) : null}

          {canLoadMore && (
            <div className="mt-3 flex justify-center">
              <Button type="button" variant="outline" size="sm" onClick={() => runSearch(page + 1, true)} disabled={isSearching}>
