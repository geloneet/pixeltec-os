...
export function TechStackMarquee() {
    ...
    return (
        ...
        >
            <div className="group flex overflow-hidden p-2 [--gap:2.5rem] [gap:var(--gap)] flex-row w-full">
                <div className="flex shrink-0 justify-around [gap:var(--gap)] animate-marquee flex-row items-center group-hover:[animation-play-state:paused]">
                    {marqueeItems.map((tech, index) => (
                        <div key={index} className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors duration-300 cursor-default">
            ...
            <div className="pointer-events-none absolute inset-y-0 left-0 w-1/12 bg-gradient-to-r from-background via-background to-transparent" />
            <div className="pointer-events-none absolute inset-y-0 right-0 w-1/12 bg-gradient-to-l from-background via-background to-transparent" />
        </div>
    );
}
