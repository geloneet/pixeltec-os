...
export function CommandPalette() {
  ...
  // Notificaciones, Perfil y seguridad) van al grupo "General".
  const navGroups = useMemo(() => {
    const byArea = new Map<NavArea, PaletteNavItem[]>();
    const general: PaletteNavItem[] = [];
          ...
          <Cmdk shouldFilter={false} className="bg-transparent flex flex-col overflow-hidden">
            ...
            <Cmdk.List className="max-h-[380px] overflow-y-auto overflow-x-hidden p-2">
              ...
              {NAV_AREA_ORDER.map((area) => {
                const items = navGroups.byArea.get(area);
              ...
              {/* General — rutas transversales sin área L1 (Archivo documental, etc.) */}
              {navGroups.general.length > 0 && (
                <Cmdk.Group heading="General" className={GROUP_CLS}>
