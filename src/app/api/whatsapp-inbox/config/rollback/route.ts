...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data, status } = await fetchPixelbot("/internal/config/rollback", {
      version,
      restored_by_uid: guard.uid,
    });
    return NextResponse.json(data, { status });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json({ error: "Error al restaurar la versión: " + message }, { status: 500 });
  }
}
