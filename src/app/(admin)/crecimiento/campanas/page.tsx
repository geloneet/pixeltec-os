...
const STATUS_COLORS: Record<string, string> = {
  ...
  strategy_ready: 'bg-cyan-500/20 text-cyan-700 dark:text-cyan-300',
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-8 lg:px-10">
      <header className="mb-8 flex items-end justify-between gap-4">
        <div>
          <h1 className="font-poppins text-3xl font-bold tracking-tight text-foreground">Campañas</h1>
      ...
      {campaigns.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border py-20 text-center">
          <Megaphone className="mb-4 h-12 w-12 text-muted-foreground/60" />
          <h3 className="font-poppins text-lg font-bold text-foreground">Sin campañas aún</h3>
              ...
              <div className="rounded-2xl border border-border bg-card p-5 transition-colors hover:bg-secondary/40">
                <div className="mb-3 flex items-start justify-between gap-2">
                  ...
                  {c.counters.totalPosts > 0 && (
                    <span className="flex items-center gap-1 font-roboto text-xs text-muted-foreground">
                ...
                <p className="mt-1 line-clamp-2 font-roboto text-xs text-muted-foreground">{c.objective}</p>
              </div>
            </Link>
          ))}
        </div>
