...
| Entidad local | `#organization` = `["Organization","ProfessionalService"]` (L1, WO-2026-00345) | ÚNICA entidad local del sitio: `telephone` raíz + `contactPoint`, `address` (localidad; `streetAddress` solo si `SITE.address.street` existe), `areaServed` = `SERVED_CITY_NAMES` + MX, `founder.url` → `/equipo`, `description` = `SITE.description`. Sin `openingHours` (no hay horario público). El bloque «Negocio local» del panel se descarta si trae LocalBusiness/ProfessionalService (`CODE_EMITTED_TYPES`); calle/horario nuevos van a `site-config`. |
| Esqueletos por página | `src/lib/seo/page-schema.ts` → `CODE_OWNED_PAGE_TYPES` | El mapa de `/seo/schema` se emite filtrando los tipos que el `page.tsx` de esa ruta (o el layout raíz, `*`) ya emite con datos reales. Filtro de EMISIÓN: el mapa guardado no cambia. Ruta o tipo nuevo emitido por código ⇒ añadirlo aquí. |
