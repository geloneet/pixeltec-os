...
Destino: PixelTEC OS `feature/dashboard-cleanup-blog` (base `f87d0e2`). Ruta admin decidida por el SG (D-A, SC-1): `src/app/(admin)/blog-cms/**` → `/blog-cms`, etiqueta «Blog». Modelo de datos: **D-C pendiente de Miguel** (este documento es el insumo).
