...
- **Editor** (`src/components/blog/cms/editor.tsx`) sobre `RichMarkdownEditor` (Tiptap ya existente): las 13 acciones de Encino equivalen a los botones ya soportados (H2/H3/¶/negrita/cursiva/listas/cita/código/enlace/imagen/tabla/deshacer/rehacer) más una acción nueva: **subida de imagen del cuerpo** (`onUploadImage`, nuevo prop) a `/api/blog/image` (mismo endurecimiento que la portada: admin, sin SVG, 5 MB, magic bytes). Inspector de 3 pestañas (Publicar · Contenido · SEO) — la pestaña «Snippets» de Encino depende de su módulo SEO Control Center, ausente en PixelTEC OS: **no portada, documentada como no implementada por dependencia ausente**, no inventada.
